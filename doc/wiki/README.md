...
`DEBUG = True`, an in-code development secret key, and an empty
`ALLOWED_HOSTS`; these settings are for local development, not production.
...
- The project configuration is development-oriented (`DEBUG = True`, a hard-
  coded secret key, empty `ALLOWED_HOSTS`, SQLite, and debug media serving).
